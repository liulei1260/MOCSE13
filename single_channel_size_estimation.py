...
parser.add_argument('--ratio', type=int, default=8, help='input output size ratio')
...
parser.add_argument('--scale', type=float, default=10., help='factor for scaling the density map pixel values for easy training')

parser.add_argument('--modelType', type=str, default='headCount_resnet50',
...
class ShanghaiTechDataset(Dataset):
    ...
    def __getitem__(self,idx):        
        ...
        zs= [float(point['Size']) for point in label]
...
scipy.io.savemat('./results_'+args.modelType+'_sh'+args.dataset+'_size'+'.mat', mdict={'pred': pred, 'mse': mse, 'mae': mae,'mre': mre,'densityArray':densityArray})
torch.save(model.state_dict(), model_dir+'headCount_'+args.modelType+'_sh'+args.dataset+'_size'+'.pt')
