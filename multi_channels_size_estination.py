...
from generate_density_map import generate_multi_density_map,generate_density_map,generate_density_size_map
...
parser.add_argument('--optimizer', type=str, default='RMSprop')  ##default='Adam'

parser.add_argument('--batchsize', type=int, default=8)  ##default=8

parser.add_argument('--ratio', type=int, default=4, help='input output size ratio')

parser.add_argument('--patchsize', type=int, default=128)  ##default=128
...
parser.add_argument('--modelType', type=str, default='headCount_vgg16_bn',
                    help='model types: headCount_vgg16, headCount_vgg16_bn,      headCount_vgg16_part')  ###default='headCount_vgg16_bn',

parser.add_argument('--cl', action='store_true', help='use curriculum loss')
                            
parser.add_argument('--numOflabel', type=int, default=3)
...
class ShanghaiTechDataset(Dataset):
    ...
    def __getitem__(self,idx):        
        ...
        label = json.load(ff)
        temp=[]
        for j in range(args.numOflabel):
            if j == 0:
                labelClass = '_carrot'
            elif j == 1:
                labelClass = '_perry'
            ...
            check=0;
            for point in label:
                #if point['label'] == labelClass:    ###before
                if labelClass in point['label']:     ###after 
                    check=1;
                    break;
            if check==1:
                #xs = [point['x'] * 100 for point in label if point['label'] == labelClass]    ###before
                #ys = [point['y'] * 100 for point in label if point['label'] == labelClass]    ###before
                xs = [point['x'] for point in label if labelClass in point['label']]    ###after
                ys = [point['y'] for point in label if labelClass in point['label']]    ###after
                zs= [float(point['Size']) for point in label if labelClass in point['label']]
                xxxx = list(zip(xs, ys, zs))
                annPoints = np.asarray(xxxx)
                if args.dataset == 'u18':
                    annPoints = annPoints['annPoints']
                # else:
                # annPoints = annPoints['image_info'][0][0][0][0][0] # for shanghaitech
                positions = generate_density_size_map(shape=image.shape, points=annPoints, f_sz=15, sigma=4)
                temp.append(positions)
            else:
                positions = np.zeros(image.shape[0:2])
                temp.append(positions)
        positionsAll=np.asarray(temp)   ####positionsAll.shape =(3, 512, 512)
        #else:
            #annPoints = annPoints['image_info'][0][0][0][0][0] # for shanghaitech
        ...
        if self.phase == 'train':
            patchSet, countSet = getRandomPatchesFromImage(image,positionsAll,targetSize,numPatches)
        ...
        if self.phase == 'val' or self.phase == 'test':
            patchSet, countSet = getAllFromImage(image, positionsAll)
            patchSet[0,:,:,:] = self.transform(np.uint8(patchSet[0,:,:,:]).transpose(1,2,0)) 
        return patchSet, countSet

def getRandomPatchesFromImage(image,positionsAll,target_size,numPatches):
    ...
    if np.random.random()>0.5:
        ...
        for j in range(args.numOflabel):
            ...
            temp.append(positions)
        positionsAll=np.asarray(temp)
    patchSet = np.zeros((numPatches,3,target_size[0],target_size[1]))
    # generate density map
    countSet = np.zeros((numPatches,args.numOflabel,target_size[0],target_size[1]))
    for i in range(numPatches):
        ...
        # density map
        for j in range(args.numOflabel):
            positions=positionsAll[j,:,:] ##append method
...
def getAllFromImage(image,positionsAll):
    nchannel, height, width = image.shape
    patchSet =np.zeros((1,3,height, width))
    patchSet[0,:,:,:] = image[:,:,:]
    countSet = positionsAll.reshape((1,positionsAll.shape[0],positionsAll.shape[1], positionsAll.shape[2]))
    return patchSet, countSet
...
num_workers=2  #gpus=2 num_works=4
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train']:
            ...
            for index, (inputs, labels) in enumerate(dataloaders[phase]):
                ...
                labels3 = labels3.view(-1,labels3.shape[2],labels3.shape[3],labels3.shape[4])
                inputs = inputs.float()
            ...
            epoch_loss = running_loss / dataset_sizes[phase]/args.numOflabel
...
def test_model(model,optimizer,phase):
    ...
    mre = 0
    checkEmpty=0 ##test
    pred = np.zeros((3000,2, args.numOflabel))
    densityArray=[]
    # Iterate over data.
    for index, (inputs, labels) in enumerate(dataloaders[phase]):
        inputs = inputs.to(device)
        ...
        labels = labels.float()
        inputs = inputs.view(-1,inputs.shape[2],inputs.shape[3],inputs.shape[4])
        labels = labels.view(-1,labels.shape[2],labels.shape[3],labels.shape[4])
        # zero the parameter gradients
        optimizer.zero_grad()
        # forward
        # track history if only in train
        with torch.set_grad_enabled(False):
            ...
            pred_count = outputs3.sum(axis=1).sum(axis=1)
        true_count = labels.to(torch.device("cpu")).numpy().sum(axis=0).sum(axis=1).sum(axis=1)
        sq=0
        ab=0
        ab2=0
        for indexLabel in range(args.numOflabel):
            sq=sq+np.square(pred_count[indexLabel] - true_count[indexLabel])
            ab=ab+np.abs(pred_count[indexLabel] - true_count[indexLabel])
            if true_count[indexLabel]==0:
                ab2=ab2+0;
                checkEmpty=checkEmpty+1
            else:
                ab2=ab2+np.abs(pred_count[indexLabel] - true_count[indexLabel])/true_count[indexLabel]
        mse = mse + sq
        mae = mae + ab
        mre = mre + ab2
    ...
    pred = pred[0:index+1,:,:]
    mse = np.sqrt(mse/(index+1)/args.numOflabel)
    mae = mae/(index+1)/args.numOflabel
    mre = mre/(((index+1)*args.numOflabel)-checkEmpty)
    print(phase+':')
    print(mae,mse,mre)
    time_elapsed = time.time() - since
...
scipy.io.savemat('./results_'+args.modelType+'_size'+'.mat', mdict={'pred': pred, 'mse': mse, 'mae': mae,'mre': mre,'densityArray':densityArray})
torch.save(model.state_dict(), model_dir+'headCount_'+args.modelType+'_size'+'.pt')
