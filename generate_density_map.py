...
def generate_density_map(shape=(5,5),points=None,f_sz=15,sigma=4):
    ...
    for j in range(len(points)):
        ...
        im_density[y1-1:y2,x1-1:x2] = im_density[y1-1:y2,x1-1:x2] +  H;  ##H*scale mark...
    return im_density
    
    
def generate_density_size_map(shape=(5,5),points=None,f_sz=15,sigma=4):
    """
    generate density map given head coordinations
    """
    im_density = np.zeros(shape[0:2])
    ...
    for j in range(len(points)):
        ...
        if change_H:
            H =  matlab_style_gauss2D((y2h-y1h+1,x2h-x1h+1),sigma)
        im_density[y1-1:y2,x1-1:x2] = im_density[y1-1:y2,x1-1:x2] +  H*z;  ##H*scale mark...
    return im_density
